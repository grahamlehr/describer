...
responses in ``tests/fixtures`` without touching the network.
"""
...
class RailApiError(RuntimeError):
    """Any failure to obtain a usable board from the API."""
...
def _delay_minutes(scheduled: str | None, expected: str | None) -> int:
    """Whole minutes between two "HH:MM" board times, clamped at zero."""
    if not scheduled or not expected:
        return 0
    if not (_HHMM_RE.match(scheduled) and _HHMM_RE.match(expected)):
        return 0
